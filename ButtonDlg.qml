...
Button {
    ...
    background: Rectangle {
        function getColor() {
            if (!enableOnPressIndicate) {
                return (control.on ?
                            (control.down ? Qt.darker(control.backgroundColorOn, 1.5) : control.backgroundColorOn)
                          : (control.down ? Qt.darker(control.backgroundColorOff, 1.5) : control.backgroundColorOff));
            } else {
                if (trigger_press) {
                    return control.backgroundColorOn;
                } else {
                    return (control.on ?
                                (control.down ? Qt.darker(control.backgroundColorOn, 1.5) : control.backgroundColorOn)
                              : (control.down ? Qt.darker(control.backgroundColorOff, 1.5) : control.backgroundColorOff));
    ...
    contentItem: Text {
        ...
        font {
            pixelSize: 16
            bold: control.textBoldByOn ?
                      ((!control.on) ? false : true)
                    : control.font.bold
        }
        opacity: enabled ? 1.0 : 0.3
        color: control.down ?
                   ( (control.on) ? Qt.darker(control.textColorOn, 1.5) : Qt.darker(control.textColorOff, 1.5))
                 : ( (control.on) ? control.textColorOn : control.textColorOff)
